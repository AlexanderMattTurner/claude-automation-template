...
# functions IN-PROCESS; one end-to-end test below still invokes it via the CLI.
...
def test_flags_then_clean(tmp_path):
    rc, err = _run(
        tmp_path / "a",
        {"docs/x.md": "See `setup-overlay.sh:121-146` for the merge.\n"},
    )
    assert rc == 1
    assert "docs/x.md" in err
    assert "setup-overlay.sh:121-146" in err

    rc, err = _run(
        tmp_path / "b",
        {"docs/x.md": "See the `.mcpServers` merge in setup-overlay.sh.\n"},
    )
    assert rc == 0, err
...
FLAGGED = {
    "file_ext_single": "cite `scripts/version-bump.sh:17` here\n",
    "file_ext_range": "cite `hooks/setup-overlay.sh:92-111` here\n",
    ...
    "tilde_colon": "in entrypoint.sh ~:762 the scrub runs\n",
...
CLEAN = {
    ...
    "osi_layer_paren": "the allow-probe does a bare TCP connect (L4)\n",
    ...
    "plain_file_ref": "extend the merge in `setup-overlay.sh` as needed\n",
