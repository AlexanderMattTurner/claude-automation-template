...
from tests._helpers import REPO_ROOT, hook_exclude_re
...
def test_prose_flag_forces_prose_mode(tmp_path: Path) -> None:
    """A PR-body-style scan feeds a code-less text file with --prose; without the
    flag the same non-prose suffix is scanned comment-only and stays silent."""
...
def test_txt_is_not_prose(tmp_path: Path) -> None:
    """.txt is data (a wordlist carries "graceful" as a dictionary word) — it
...
# process coverage tooling cannot trace — so drive main() directly here for the
# CLI path (mode selection, exit-code contract, the read-failure skip).
...
def _is_scanned(path: Path) -> bool:
    """Approximate the hook's `types_or: [shell, python, javascript, markdown,
    yaml]` selection: by suffix, or an extensionless script whose shebang names a
    shell/python/node interpreter."""
    if path.suffix in {
        ".sh",
        ".bash",
        ".py",
...
def test_enforced_scope_is_clean() -> None:
    ...
    exclude = hook_exclude_re("check-graceful-handwave")
