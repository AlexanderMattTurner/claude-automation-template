...
@pytest.mark.parametrize(
    ...
    [
        ...
        "    # renamed from OLD_VAR to NEW_VAR",
...
def test_main_wires_violations_and_message(
    ...
    """main() runs the detector over argv paths and prints its message per hit."""
    bad = tmp_path / "bad.sh"
...
def test_main_skips_an_unreadable_path(tmp_path: Path) -> None:
    assert mod.main([str(tmp_path / "absent.sh")]) == 0


def _run_script(*paths: str) -> subprocess.CompletedProcess:
...
def _is_scanned(path: Path) -> bool:
    ...
    if path.suffix in (".bash", ".sh", ".py", ".mjs", ".cjs", ".js"):
        return True
    if path.suffix:
        return False
    try:
...
def test_enforced_scope_is_clean() -> None:
    ...
    exclude = hook_exclude_re("check-historical-comments")
    ...
    offenders = []
    for rel in tracked:
        if not rel or exclude.match(rel):
            continue
        path = REPO_ROOT / rel
