...
- **Docker Compose `command:`/`entrypoint:` env vars: use `$$VAR`, not `$VAR`.** Compose interpolates `$VAR` at config-render time using the _host_ environment; an unset host var silently becomes `""`. `$$VAR` defers expansion to the container shell. `docker compose config` surfaces the problem as a “variable is not set, defaulting to blank” warning.
...
- **When a hook’s fail-closed/error path is only exercised through its spawned-subprocess CLI, add an in-process seam** (e.g., env-var-overridable sentinel path) so the branch is mutation-testable, not just line-covered. Stryker can’t trace into a child interpreter, so a security-critical catch block can sit at 0% killed while c8 reports it covered.
...
- **Installer/bootstrap scripts tested only on one OS leave cross-OS divergence invisible.** Add a CI matrix over every target OS, and test each written path through hostile pre-states (especially a dangling symlink)—not just the happy path. (`mkdir -p` silently succeeds on a dangling symlink under BSD but errors under GNU.)
- **When relocating a gate earlier in a script, update every test that bypassed the earlier position to reach a later guard.** A test that stripped a tool from PATH to isolate a downstream check will now fire on the newly hoisted gate instead, measuring the wrong thing.
