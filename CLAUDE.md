...
- **Order redirections left-to-right: place `2>/dev/null` before an input redirect when suppressing open failures.** Shells apply redirections left-to-right, so `cmd <file 2>/dev/null` still prints "No such file" to the original stderr when the open fails — fd 2 isn’t redirected yet. Write `cmd 2>/dev/null <file` instead.
- **A best-effort stage in a pipeline must exit 0 unconditionally.** A script in `producer | writer | consumer` that exits non-zero on its last operation (e.g., a trailing `[[ … ]] && …` that finds nothing) masks the producer’s failure when the caller reads `PIPESTATUS[0]`. End every best-effort writer with an explicit `exit 0` and add a test for the empty-output path.
...
- **In Node.js, `try/catch` around `child_process.spawn` does not catch a missing executable.** `ENOENT` for an unresolvable command arrives as an asynchronous `’error’` event on the child, not a synchronous throw — so the `catch` block is never entered and an unhandled `’error’` crashes the whole process. Any best-effort `spawn` (browser opener, notifier, optional helper) must attach `child.on(‘error’, () => {})` regardless of the surrounding `try/catch`.
...
- **When a static model enumerates a boundary set, the runtime test must probe every member — not one representative.** Drive the test from the same SSOT list the enumeration uses so re-allowing any single member fails the live check. Checking only entry 0 of a 9-entry deny-list leaves entries 1–8 executed but unverified.
- **Prefer a structural or count invariant over a wall-clock budget for performance regression tests.** A differential metric (warm − cold latency) is blind to a cost added to both sides. Count a deterministic structural quantity instead (e.g., “zero registry round-trips on a warm run”) — it’s unambiguous and environment-proof. Also ensure the test exercises the branch that enables the optimization; a guard that never enters the fast path passes for the wrong reason.
- **A diagnostic or health-check command must reproduce the exact parameters of the real path it claims to verify.** A check that uses broader scope (e.g., an unscoped credential where the real path uses a repo-scoped one) can pass on the very failure it’s meant to explain. Build diagnostics from the same shared helper as the real path; a green diagnostic that doesn’t match the red is worse than no check.
- **Point `XDG_DATA_HOME`, `XDG_CONFIG_HOME`, and `XDG_CACHE_HOME` at throwaway directories in test setup.** A test that invokes a CLI tool with real XDG state reads and writes the developer’s actual cache and config, making the suite non-hermetic. Use a session-scoped fixture that redirects all three to a temp dir and tears it down after the suite.
