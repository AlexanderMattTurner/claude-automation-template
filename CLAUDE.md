...
- **Required checks: gate on an `if: always()` summary job, never the underlying job.** A skipped or cancelled job posts no status, leaving PRs stuck “pending” forever. Add a summary job (`needs:` the real jobs **and** `decide`, `if: always()`, fails on failure/cancelled) and mark that Required instead. Give each summary job a distinct name (branch protection matches by name).
- **Don’t path-gate a Required workflow with a workflow-level `paths:` filter—use a `decide` job instead.** A `paths:` filter skips the whole workflow (summary gate included), so the Required check never reports and the PR hangs forever. Instead run the workflow unconditionally and add an always-running `decide` job that calls `.github/scripts/paths-changed.sh <git-pathspec…>` (it diffs `BASE_SHA...HEAD_SHA` and outputs `run=true|false`, failing **open** when the range is undeterminable). Gate the real job on `if: needs.decide.outputs.run == 'true'`; a `decide`-driven skip is not a failure, so the summary gate still passes. List the workflow’s own file and `paths-changed.sh` itself in its pathspecs. This keeps the CI-time savings of path gating while staying safe to mark Required.
