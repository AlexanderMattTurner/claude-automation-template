...
- **A rule the session must obey under execution momentum belongs on the tool-use loop, not in prose.** An instruction in `CLAUDE.md` or a skill competes with whatever the session is already part-way through doing, and loses; a `PreToolUse` hook that denies the raw call and names the sanctioned path in its reason does not. Say the ceiling in the hook's header: a hook that lives in the tree the session edits is a speed bump, not a boundary. Give it the `ask` posture, which puts a human in the loop, when the gate has to hold.
