...
- **Skip the Lessons Learned section entirely when the repo this PR MERGES INTO is the template.** phone-home runs in the base repo, so read that name — the PR's base repo from a fork, `git remote get-url origin` otherwise — and never the repo's own `CLAUDE.md` prose. Compare it with the repo `TEMPLATE_REPO` names in `.github/workflows/phone-home.yaml`, which `vars.TEMPLATE_SYNC_ORG` redirects for a fork-hosted template. Every other repo only sends and never receives, so dropping the section there loses the lesson.
