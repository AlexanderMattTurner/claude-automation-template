...
- **Don't inline a substantial shell script in a `run:` block — extract it to `.github/scripts/<name>.sh` and call `run: bash .github/scripts/<name>.sh`.** Inline shell is invisible to the repo's shell linters (shellcheck and shfmt only see standalone `.sh` files), so a long inline block ships unchecked. Rule of thumb: more than ~10 lines, or any branching logic, gets extracted; trivial glue (one command, one output assignment) stays inline. **A script a composite action runs lives in that action's own directory**, called as `bash "${GITHUB_ACTION_PATH}/<name>.sh"`, with its tests and data files beside it — a caller then sparse-checks out one directory to get the whole action, and a consumer cannot receive the action without the scripts it runs. **A job calling an external script must `actions/checkout` first** — a checkout-less decide or report job either sparse-checks out the directory that holds the script or keeps the small block inline.
