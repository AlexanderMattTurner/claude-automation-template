...
- **actionlint does not parse composite actions, so their inline `run:` shell is checked by nothing.** Transpose each composite's `runs.steps` to a synthetic workflow (prefix the header with exactly as many blank lines as the action's own header occupies so diagnostics point at the real file, then re-indent the steps under a synthetic `jobs.<id>.steps:`) and run actionlint over it in the job that already invokes actionlint. Declare the composite's `inputs` in YAML flow style on the synthetic trigger so `inputs.*` references do not generate "property not defined" noise that buries real findings. When a composite action wraps a third-party action, assert that every input it forwards is one the pinned version still accepts — an unknown input silently becomes a `##[warning]` and is discarded.
- **`node --test`'s default discovery skips dot-directories**, so `.github/scripts/*.test.mjs` never run under a bare `node --test`. Pass the directory explicitly (`node --test '.github/scripts'`) or keep test files outside `.github/`.
...
- **`gh api` refuses a response body that contains raw terminal escape bytes unless `--allow-escape-sequences` is passed** — the same guard as `gh pr diff`, and it fires regardless of whether stdout is a terminal.
...
- **A whole-tree lint can flag its own docstring examples and fixture files.** Keep doc examples in a form the extractor drops (a placeholder or generic non-namespaced name); give fixture files names the classifier reads as uses rather than definitions.
