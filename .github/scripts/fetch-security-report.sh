...
if [[ -f package.json ]]; then
  # Capture output and exit code directly rather than piping through `head -100`
  # and reading PIPESTATUS: when audit output exceeds 100 lines, `head` closes
  # the pipe early and pnpm can be SIGPIPE-killed (exit 141) instead of exiting
  # with its real 0/1, which would misreport a clean or expected-vulnerabilities
  # run as "encountered an error".
  pnpm_output=$(pnpm audit 2>&1)
  pnpm_rc=$?
  printf '%s\n' "$pnpm_output" | awk 'NR <= 100' >>"$REPORT_PATH"
