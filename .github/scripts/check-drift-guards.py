...
smell unless a true single source of truth is infeasible (an external value you
don't control, a cross-language boundary that genuinely prevents sharing). This
lint makes that judgement auditable: any test whose name or docstring uses
drift-guard intent ("drift guard", "can't drift", "must stay in sync", …) MUST
carry
...
slips through, like any heuristic check-* lint.
"""
...
# in unrelated tests, so we require the copies-agree phrasings ("in lockstep",
# "kept in sync"), not just the word.
_GUARD_PATTERNS = (
    r"drift[- ]guard",
    r"anti[- ]?drift",
