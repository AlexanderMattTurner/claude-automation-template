...
EXCLUDED on purpose: they are overwhelmingly present-tense state, not history —
"the container is no longer running", "the image for this commit", "the value is
used to authenticate", "the previously cached digest". Gating on them would flag
legitimate comments, so they are left to review. This check guards the unambiguous
class so it can't reappear.
...
and user-facing copy are spared.
...
def main(argv: list[str]) -> int:
    ...
    for path in argv:
        ...
        for lineno in violations(text):
            print(f"{path}:{lineno}: {_MESSAGE}", file=sys.stderr)
            status = 1
    return status
