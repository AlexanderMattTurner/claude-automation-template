...
Line-number citations rot the instant code moves: `setup.sh:121-146`, `(L139)`,
...
excluded: its released entries are an immutable audit record (and `(L4)` there can
mean OSI layer 4, not a line). Agent/skill instruction files under `.claude/` and
prompt templates under `.github/` are out of scope — they carry deliberately
illustrative `path/to/file.py:42` placeholders.
