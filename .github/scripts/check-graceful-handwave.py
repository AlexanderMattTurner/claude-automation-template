...
`--prose` forces prose mode for every PATH (for a free-standing document with no
code extension, e.g. a PR body); otherwise the mode is chosen per file by extension.
"""
...
# .txt is deliberately NOT prose: .txt files are typically data (a wordlist
# carries "graceful" as a dictionary word, not a claim).
